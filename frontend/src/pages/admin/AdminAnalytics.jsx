...
const SYS_HEALTH = [
  ...
  { label: 'RAM',     value: 61,  unit: '%',  max: 100 },
  { label: 'Disk',    value: 45,  unit: '%',  max: 100 },
  { label: 'Network', value: 120, unit: 'ms', max: 200 },
];
...
export default function AdminAnalytics() {
  return (
    <div style={wrap}>
      ...
      <div style={card}>
        <div style={{ ...cardTitle, marginBottom: 16 }}>System Health</div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 16 }}>
          {SYS_HEALTH.map(({ label, value, unit, max }) => (
            <div key={label}>
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, marginBottom: 6 }}>
                <span style={{ color: 'var(--text-muted)' }}>{label}</span>
                <span style={{ color: 'var(--accent)', fontWeight: 700 }}>{value}{unit}</span>
              </div>
              <div style={{ height: 5, background: 'rgba(255,255,255,0.06)', borderRadius: 99, overflow: 'hidden' }}>
                <div style={{ height: '100%', width: `${(value / max) * 100}%`, background: 'var(--accent)', borderRadius: 99 }} />
              </div>
            </div>
          ))}
        </div>
        ...
        <div style={card}>
          ...
          <ResponsiveContainer width="100%" height={190}>
            <AreaChart data={weeklyData}>
              <defs>
                <linearGradient id="adminTGrad" x1="0" y1="0" x2="0" y2="1">
              ...
              <YAxis             tick={tick} axisLine={false} tickLine={false} />
              <Tooltip contentStyle={ttStyle} />
              <Area type="monotone" dataKey="tokens" stroke="#ec4899" fill="url(#adminTGrad)" strokeWidth={2} />
            </AreaChart>
          </ResponsiveContainer>
        </div>
      </div>
